...
<div id="reply" class="small_panel ui-widget-content ui-corner-all">
	<h3 class="ui-widget-header ui-corner-all">留言回复</h3> 
    <table>
      <tr class="label">
        <td>光顾者</td>
        <td>操作</td>
      ...
      <?php
        $comment->get_user_comments('on', 'replygood', $user_id, 0, 100);
      ...
      <tr>
          <td><h4><?php
                    ...
                    $user->get_user();?>
                    <a href="../user_good_list?id=<?php echo $user->get_id(); ?>"><?php $username=$user->get_name();?></a></h4>
              <img src="<?php $user->get_photo(); ?>" />
        </td>
        <td>
           <button type="button" class="reply_but" touserid="<?php echo $user->get_id(); ?>" tousername="<?php echo $user->get_name();  ?>" kindl="replygood" goodid="<?php echo $goodid; ?>" fromname="<?php echo $user_name; ?>" fromid="<?php echo $user_id ?>">回复</button>
           <button type="button" class="del_comment" name="<?php echo $comment->get_id(); ?>">删除</button>
        </td>
        <td><a href="../good?id=<?php echo $good->get_id(); ?>" style="color:blue; font-weight:bold;"><?php $good->get_name(); ?></a></td>
      ...
      <?php
        ...
        ?>
		<?php
        $comment->get_user_comments('on', 'replylook', $user_id, 0, 100);
        for($i=0;$i<$comment->return_count();$i++){
          $comment->get_comment('comments', $i);
      ?>
      <tr>
        ...
        <td>
           <button type="button" class="reply_but" touserid="<?php echo $user->get_id(); ?>" tousername="<?php echo $user->get_name();  ?>" kindl="replylook" goodid="<?php echo $goodid; ?>" fromname="<?php echo $user_name; ?>" fromid="<?php echo $user_id ?>">回复</button>
           <button type="button" class="del_comment" name="<?php echo $comment->get_id(); ?>">删除</button>
        </td>
        <td><a style="color:blue; font-weight:bold;" href="../lookfor?id=<?php echo $lookfor->get_id(); ?>"><?php echo $lookfor->get_name(); ?></a></td>
        <td><?php $comment->get_cotent(); ?></td>
      </tr>  
