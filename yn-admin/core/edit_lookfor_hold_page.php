...
//计算出总页数
//lookfor总数量
$num=$base->get_count("select count(*) from lookfor where status='hold'");
//每页数目
...
?><table>
    <tr class="label"><td>名称</td><td>创建日期</td><td>操作</td><td>回复数</td></tr>
      <?php
	   $lookfor->get_good_all_lookfor('hold',$pages_each*($page_cur-1),$pages_each);
        for($i=0;$i<$page_last;$i++){
          $lookfor->get_lookfor('lookfors', $i);
      ?>
      <tr><?php //echo $lookfor->get_id();?>
          <td><?php echo $lookfor->get_name();?></td>
