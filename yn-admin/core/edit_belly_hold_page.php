...
//计算出总页数
//lookfor总数量
$num=$base->get_count("select count(*) from goods where status='hold'");
//每页数目
...
?>
<table>
<tr class="table_topper"><td>选择</td><td>宝贝</td><td>操作</td><td>发布者</td><td>留言数</td></tr>
     <?php
	   $good->get_goods('hold', $pages_each*($page_cur-1), $pages_each);
    ...
    <tr>
      ...
      <td><button class="a" hr="../good?id=<?php echo $good->get_id(); ?>" type="button">查看</button>&nbsp;<button class="a" hr="belly_edit_panel?good_id=<?php echo $good->get_id(); ?>" type="button">编辑</button>
      &nbsp;<button class="huifu" name="<?php echo $good->get_id(); ?>">恢复</button>
      </td>
