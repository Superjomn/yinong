...
<div id="add_belly" class="small_panel ui-widget-content ui-corner-all">
  <h3 class="ui-widget-header ui-corner-all">修改宝贝信息</h3>
  <table>
      <tr>
          <td class="label">宝贝名称</td>
          <td colspan="4">
              <input name="name" value="<?php $good->get_name();// $good->get_owner(); ?>" type="text"/>
          </td>
      </tr>
    <tr>
  ...
  <div style="display:none">
      <input name="good_id" value="<?php echo $good->get_id(); ?>"/>
      <input name="owner" value="<?php $good->get_owner();  ?>"/>
      <input name="ownerid" value="<?php echo $good->get_ownerid();  ?>"/>
  </div>
  <tr id="img_show"><td></td>
   ...
   <tr>
     ...
     <td colspan="4">
         <input type="text" name="tag" value="<?php $good->get_tag(); ?>" />
     </td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td colspan="4">
         <button type="button" id="draft">保存</button>&nbsp;&nbsp;
...
editor.prototype = {
	setOptions: function(options) {
		this.options = {
			emotion: [
				{ 'title': '微笑', 'pos': '-5px -5px',  'url': co.Root + 'ok_button_r2_c2.gif' }, 
				{ 'title': '大笑', 'pos': '-32px -5px', 'url': co.Root + 'ok_button_r2_c3.gif' },
				{ 'title': '窃笑', 'pos': '-59px -5px', 'url': co.Root + 'ok_button_r2_c4.gif' },
				{ 'title': '眨眼', 'pos': '-86px -5px', 'url': co.Root + 'ok_button_r2_c5.gif' },
				{ 'title': '吐舌', 'pos': '-113px -5px','url': co.Root + 'ok_button_r2_c11.gif'},
				{ 'title': '色色', 'pos': '-140px -5px','url': co.Root + 'ok_button_r2_c6.gif' },
				{ 'title': '呲牙', 'pos': '-168px -5px','url': co.Root + 'ok_button_r2_c7.gif' },
				{ 'title': '讨厌', 'pos': '-194px -5px','url': co.Root + 'ok_button_r2_c8.gif' }
			],
			baroverOpc: 0.7
		};
	...
	setPopInit: function(command, tar) {
		...
		if(command == 'createLink' || command == 'insertImage' || command == 'insertEmotion') { // 创建链接 + 插入图片 + 插入表情形体类似. 只需要单独定制id和title即可
			...
			if(command == 'createLink' || command == 'insertImage') {
				...
				_body += '<img id="' + cfg.cofbtnId + '" style="padding-right:10px;" src="'+co.Root+'ok_button.gif" />';
				_body += '<img id="' + cfg.celbtnId + '" src="'+co.Root+'cel_button.gif" /></div>';
			}
			if(command == 'insertEmotion') {
				for(var i = 0, l = this.emotion.length; i < l; i++) {
					...
					_td += '<td unselectable="on" url="'+this.emotion[i].url+'" title='+this.emotion[i]['title']+' style="cursor:pointer;background-image:url('+co.Root+'ok_button.gif)'+S+'"></td>';
				}
		...
		if(command == 'insertHTML') {
			...
			_body += '<img id="'+cfg.celId+'" src="'+co.Root+'cel_button.gif" /></div></div>';
		}
	...
	createIframe: function() {
		...
		this.doc.write("<html><head><style>body{ margin:3px; word-wrap:break-word; word-break: break-all; }</style></head><body><?php $good->get_description(); ?></body></html>");
