...
include("phpmailer/class.smtp.php"); // 可选    


function sent($user,$add,$subject,$content){
	$mail              = new PHPMailer();    
...
$mail->FromName    = "易农易购";              //必填，发件人昵称或姓名    
$mail->Subject     = "'$subject'";       //必填，邮件标题（主题）    
$mail->AltBody     = "Yinong"; //可选，纯文本形势下用户看到的内容    
$mail->WordWrap    = 50; // 自动换行的字数    
   
$mail->MsgHTML($body);    
   
$mail->AddReplyTo("redcross_cau@126.com","Webmaster");//回复邮箱地址       
$mail->AddAddress("$add","$user");//参数一：收信人的邮箱地址，可添加多个。参数二：收件人称呼    
...
//sent('superjom','superjom@gmail.com','你好啊','<body>你好，函数的实验</body>');
